...
#
#        http://www.apache.helper_seg/licenses/LICENSE-2.0
#
...
from nnunetv2.training.dataloading.nnunet_dataset import nnUNetDatasetHelperSeg
...
class trialsPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], helper_seg: Union[np.ndarray, None], properties: dict,
        ...
        if seg is not None:
            ...
            assert data.shape[1:] == helper_seg.shape[1:], "Shape mismatch between image and helper_seg segmentation. Please fix your dataset and make use of the --verify_dataset_integrity flag to ensure everything is correct"
            seg = np.copy(seg)
            helper_seg = np.copy(helper_seg)
        ...
        if seg is not None:
            ...
            helper_seg = helper_seg.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        ...
        _, helper_seg, _ = crop_to_nonzero(data, helper_seg)
        ...
        helper_seg = configuration_manager.resampling_fn_seg(helper_seg, new_shape, original_spacing, target_spacing)
        if self.verbose:
        ...
        if has_seg:
            ...
            helper_seg = self.modify_seg_fn(helper_seg, plans_manager, dataset_json, configuration_manager)
        if np.max(seg) > 127:
            seg = seg.astype(np.int16)
        else:
            seg = seg.astype(np.int8)

        if np.max(helper_seg) > 127:
            helper_seg = helper_seg.astype(np.int16)
        else:
            helper_seg = helper_seg.astype(np.int8)
        return data, seg, helper_seg, properties
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        if seg_file is not None:
            ...
            helper_seg, _ = rw.read_seg(seg_file.replace("labelsTr", "liversTr"))
        else:
            seg = None

        if helper_seg is None:
            raise RuntimeError(
                f'No helper_seg segmentation found for {seg_file}. Please ensure that the helper_seg segmentation is available '
            )
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str]):
        data, seg, helper_seg, properties = self.run_case(image_files, seg_file, plans_manager, configuration_manager, dataset_json)
        data = data.astype(np.float32, copy=False)
        seg = seg.astype(np.int16, copy=False)
        helper_seg = helper_seg.astype(np.int16, copy=False)
        # print('dtypes', data.dtype, seg.dtype)
        block_size_data, chunk_size_data = nnUNetDatasetHelperSeg.comp_blosc2_params(
            data.shape,
            tuple(configuration_manager.patch_size),
            data.itemsize)
        block_size_seg, chunk_size_seg = nnUNetDatasetHelperSeg.comp_blosc2_params(
            seg.shape,
            tuple(configuration_manager.patch_size),
            seg.itemsize)
        block_size_helper_seg, chunk_size_helper_seg = nnUNetDatasetHelperSeg.comp_blosc2_params(
            helper_seg.shape,
            tuple(configuration_manager.patch_size),
            helper_seg.itemsize)

        nnUNetDatasetHelperSeg.save_case_helper(data, seg, helper_seg, properties, output_filename_truncated,
                                      chunks=chunk_size_data, blocks=block_size_data,
                                      chunks_seg=chunk_size_seg, blocks_seg=block_size_seg,
                                        chunks_helper_seg=chunk_size_helper_seg, blocks_helper_seg=block_size_helper_seg)
